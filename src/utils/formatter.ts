 ...
 */
export function mapColorTagToObsidianTag(
  colorName: string,
 ...
 */
function formatHighlightWithComments(
  highlight: Highlight,
  tagMapping: Record<string, string>,
): string[] {
  const lines: string[] = [];
...
export function formatExtraction(
  ...
  const processedHighlights = options.deduplicate ? deduplicateHighlights(highlights) : highlights;
